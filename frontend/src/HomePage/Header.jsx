...
function Header({ isLoggedIn, setIsLoggedIn, username, handleLogout }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      businesses.forEach(b => results.push({ type: 'business', name: b.name, link: `/business/${b.id}/products` }));
      products.forEach(p => {
  ...
  return (
    <header className="w-full bg-[#F5F5DC] py-4 shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center px-4 gap-4">
        <Link to="/" className="flex items-center">
          ...
          <div className="ml-2">
            <h1 className="text-2xl font-bold text-[#000000] font-poppins leading-none">EDU</h1>
            <h1 className="text-2xl font-bold text-[#000000] font-poppins leading-none">Startups</h1>
          </div>
        </Link>
          ...
          <div className="flex-1 w-full flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="relative">
              <button
                ...
                className="bg-[#3D0301] text-white px-4 py-2 rounded-lg hover:bg-[#6B1532] font-bold font-poppins text-[20px] flex items-center"
              >
